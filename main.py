...
def is_related_title(title_a: str, title_b: str, sim: float, threshold: float = 0.6) -> bool:
    """Adaptive filter for all domains, with tech-stack awareness."""
    if sim < threshold:
        return False

    a, b = title_a.lower(), title_b.lower()

    # Identify tech stacks or key skills
    tech_keywords = [
        "python", "php", "java", "javascript", "typescript", "c#", "dotnet", "react",
        "angular", "vue", "flutter", "node", "golang", "ruby", "laravel", "django",
        "fastapi", "spring", "kotlin", "swift"
    ]
    tech_a = [t for t in tech_keywords if t in a]
    tech_b = [t for t in tech_keywords if t in b]

    # If both have known techs but no overlap, they are NOT the same domain
    if tech_a and tech_b and not any(t in tech_b for t in tech_a):
        return False

    # General domain clusters
    clusters = {
        "tech": ["developer", "engineer", "programmer", "backend", "frontend", "full stack", "software", "it"],
        "design": ["designer", "editor", "video", "motion", "creative", "illustrator"],
        "marketing": ["smm", "marketing", "advertising", "seo", "content", "manager"],
        "logistics": ["dispatch", "driver", "coordinator", "truck", "fleet", "logistics"],
        "finance": ["accountant", "finance", "bank", "auditor", "economist"],
        "medical": ["doctor", "nurse", "medical", "pharma", "healthcare"],
        "education": ["teacher", "tutor", "trainer", "instructor"],
    }
    ...
    # Skip if from clearly different domains (developer vs nurse)
